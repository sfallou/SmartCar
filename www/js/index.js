...
/* global cordova, console, $, bluetoothSerial, _, refreshButton, deviceList, previewColor, red, green, blue, disconnectButton, connectionScreen, vitesseControl, rgbText, messageDiv */
'use strict';
...
var app = {
    ...
    onColorChange: function (evt) {
        var c = app.getCommande();
        rgbText.innerText = "(Start,vitesse,Direction,sens):"+c;
        //previewColor.style.backgroundColor = "rgb(" + c + ")";
        app.sendToArduino(c);
    },
    getCommande: function () {
        var valeurs = [];
        valeurs.push(start.value);
        valeurs.push(vitesse.value);
        valeurs.push(direction.value);
        valeurs.push(sens.value);
